...
public class PlayerCharacter : Character
{
    ...
    {
        if (isAiming) return;
        if (stamina < rollStamina) return;
        //if (moveRaw.magnitude <= 0) return;

        currentStaminaRecoveryDelay = 1f;
        ReduceStamina(rollStamina, false);

        //Vector3 rollVelocity = Vector3.Scale(moveRaw, rollDistance * new Vector3((Mathf.Log(1f / (Time.deltaTime * rigid.drag + 1)) / -Time.deltaTime), 0, (Mathf.Log(1f / (Time.deltaTime * rigid.drag + 1)) / -Time.deltaTime)));

        //var rollVelocity = rollDistance * transform.position.z;
        rigid.AddForce(transform.forward*rollDistance, ForceMode.VelocityChange);
        anim.Play("Roll");
    }
